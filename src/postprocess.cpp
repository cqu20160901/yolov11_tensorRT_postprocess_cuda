...
int GetResultRectYolov11::GetConvDetectionResult(std::vector<float *> &BlobPtr, std::vector<float> &DetectiontRects)
{
    int ret = 0;
    ...
    for (int i = 0; i < CoordIndex; i ++)
    {
        ...
        for (int j = 4; j < ClassNum + 4; j ++) 
        {
            if (4 == j)
            {
                MaxScore = Output[j * CoordIndex + i];
                MaxIndex = j;   
            } 
            else 
            {
                if (MaxScore <  Output[j * CoordIndex + i])
                {
                    MaxScore = Output[j * CoordIndex + i];
                    MaxIndex = j;   
                }
        ...
        if (MaxScore > ObjectThresh)
        {
            ...
            cx = Output[0 * CoordIndex + i];
            cy = Output[1 * CoordIndex + i];
            cw = Output[2 * CoordIndex + i];
            ch = Output[3 * CoordIndex + i];
